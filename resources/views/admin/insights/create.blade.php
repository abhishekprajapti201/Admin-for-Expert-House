...
<div class="w-full max-w-7xl mx-auto bg-white shadow-2xl rounded-2xl border border-gray-200 overflow-hidden">
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="px-8 py-6" id="postForm">
        ...
        <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-200/80 mt-8">
            ...
            <button type="submit" class="px-7 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white text-sm font-medium rounded-xl shadow-md hover:shadow-lg transition-all duration-200 flex items-center gap-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const commonConfig = {
        ...
        removePlugins: ['ImageInsert', 'ImageUpload', 'MediaEmbed'],
        language: 'en'
    };

    // Initialize Paragraph Editor
    let paragraphEditor = null;
    ClassicEditor
        .create(document.querySelector('#paragraph'), commonConfig)
        .then(editor => {
            paragraphEditor = editor;
            console.log('Paragraph editor initialized successfully');
        })
        .catch(error => {
            console.error('Paragraph CKEditor error:', error);
    ...
    ClassicEditor
        .create(document.querySelector('#description'), commonConfig)
        .then(editor => {
            descriptionEditor = editor;
            console.log('Description editor initialized successfully');
        })
        .catch(error => {
            console.error('Description CKEditor error:', error);
            // Fallback to textarea if CKEditor fails
    ...
    form.addEventListener('submit', function(e) {
        // Update textareas with editor content
        if (paragraphEditor) {
            const paragraphContent = paragraphEditor.getData();
            document.querySelector('#paragraph').value = paragraphContent;
        }
